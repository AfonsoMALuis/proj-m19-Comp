...
void m19::xml_writer::do_left_index_node(m19::left_index_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  openTag(node, lvl);
  openTag("pointer", lvl);
...
void m19::xml_writer::do_body_node(m19::body_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  openTag(node, lvl);
  if (node->initialSection()) {
    openTag("initialSection", lvl + 2);
    node->initialSection()->accept(this, lvl + 4);
    closeTag("initialSection", lvl + 2);
  }
  if (node->section()) {
    openTag("section", lvl + 2);
    node->section()->accept(this, lvl + 4);
    closeTag("section", lvl + 2);
  }
  if (node->finalSection()) {
    openTag("finalSection", lvl + 2);
    node->finalSection()->accept(this, lvl + 4);
